...
class COAD(collections.abc.MutableMapping):
    ...
    def __init__(self, filename=None, create_db_file=True):
        if filename is None:
            filename = (
                os.path.abspath(os.path.dirname(__file__)) + os.sep + "master.xml"
            )
        try:
            with open(filename):
                pass
    ...
    def list(self, classname):
        ...
        cur = self.dbcon.cursor()
        list_select = (
            "SELECT name FROM object WHERE class_id IN "
    ...
    def get_hierarchy_for_object_id(self, object_id):
        ...
        _logger.info(
            "get_hierarchy_for_object_id(%s) took %s sec",
            object_id,
            time.time() - stime,
        )
    ...
    def get_by_hierarchy(self, identifier, default=None):
        ...
        _logger.info(
            "get_by_hierarchy(%s) took %s sec", identifier, time.time() - stime
        )
        return retobj
...
class ClassDict(collections.abc.MutableMapping):
    ...
    def __init__(self, coad, meta):
        ...
        cur.execute(
            "SELECT collection_id, parent_class_id FROM collection WHERE child_class_id=?",
            [self.meta["class_id"]],
        )
            ...
            for row in cur.fetchall():
                ...
                prop = dict(
                    zip(
                        ...
                        [str(v) if isinstance(v, int) else v for v in row],
                    )
                )
                ...
                if prop["property_id"] in self.valid_properties:
                    raise Exception(
                        "Duplicate property %s in class %s"
                        % (prop["name"], self.meta["name"])
                    )
    ...
    def __getitem__(self, key):
        ...
        cur = self.coad.dbcon.cursor()
        cur.execute(
            "SELECT * FROM object WHERE class_id=? AND name=?",
            [self.meta["class_id"], key],
        )
        objrow = cur.fetchone()
        if objrow is None:
    ...
    def __len__(self):
        cur = self.coad.dbcon.cursor()
        cur.execute(
            "SELECT count(*) FROM object WHERE class_id=?", [self.meta["class_id"]]
        )
        return cur.fetchone()[0]
    ...
    def get_collection_id(self, child_class_id):
        ...
        cur = self.coad.dbcon.cursor()
        cur.execute(
            "SELECT collection_id FROM collection WHERE parent_class_id=? AND child_class_id=?",
            [self.meta["class_id"], child_class_id],
        )
    ...
    def get_category_id(self, category):
        ...
        cur = self.coad.dbcon.cursor()
        cur.execute(
            "SELECT category_id FROM category WHERE class_id=? AND name=?",
            [self.meta["class_id"], category],
        )
    ...
    def add_category(self, name):
        ...
        for row in cur.fetchall():
            ...
            if cat["name"] == "name":
                raise Exception(
                    "Category %s already exists in %s" % (name, self.meta["name"])
                )
    ...
    def new(self, name, category="-"):
        ...
        cur = self.coad.dbcon.cursor()
        fill = ",".join("?" * len(cols))
        cmd = "INSERT INTO object (%s) VALUES (%s)" % (
...
class ObjectDict(collections.abc.MutableMapping):
    ...
    def __init__(self, coad, meta):
        ...
        cur.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='attribute_data'"
        )
        ...
        _logger.info(
            "ObjectDict init %s took %s sec", meta["name"], time.time() - stime
        )
    ...
    def __setitem__(self, key, value):
        ...
        if key not in self.valid_attributes:
            ...
            raise Exception(
                msg % (key, self.meta["name"], self.valid_attributes.keys())
            )
        cur = self.coad.dbcon.cursor()
        ...
        vls = [
            value,
            self.meta["object_id"],
            self.valid_attributes[key]["attribute_id"],
        ]
        cur.execute(cmd, vls)
        if cur.rowcount == 0:
    ...
    def copy(self, newname=None):
        ...
        cur.execute(
            "SELECT * FROM membership WHERE parent_object_id=?",
            [self.meta["object_id"]],
        )
        ...
        for row in cur.fetchall():
            ...
            vls = (
                ...
                ",".join(["?" for d in newrow[1:]]),
            )
            cur.execute(cmd % vls, newrow[1:])
        # Copy memberships where this is the child
        cur.execute(
            "SELECT * FROM membership WHERE child_object_id=?", [self.meta["object_id"]]
        )
        cols = [d[0] for d in cur.description]
    ...
    def set_category(self, name):
        ...
        for cat in available_cats:
            if cat["name"] == name:
                cur = self.coad.dbcon.cursor()
                cur.execute(
                    """UPDATE object SET category_id=? WHERE object_id=?""",
                    [cat["category_id"], self.meta["object_id"]],
                )
                self.coad.dbcon.commit()
                return
        raise Exception(
            "No such category %s for class %s" % (name, self.get_class().meta["name"])
        )
    ...
    def get_collection_id(self, child_class_id, name=None):
        ...
        if len(rows) == 0:
            ...
            raise Exception(
                "Multiple collections available for relationship.  Choose name from %s"
                % str(namelist)
            )
        return rows[0][0]
    ...
    def set_property(self, name, value, tag="System.System", data_tag=None):
        ...
        def get_mask_value(value, mask=None):
            ...
            if mask:
                ...
                for k in it:
                    ...
                    vv.append(mval)
                raise Exception(
                    "Value '%s' not in property's input_mask.  Valid values are:\n%s\n"
                    % (value, "\n".join(vv))
                )
            else:
                return value
        ...
        if tag_clsname not in self.get_class().valid_properties_by_name:
            ...
            for ptag in possible_tags:
                ...
                if ptag_prop[0] == name:
                    ...
                    if ptag_member[0] == self.meta["object_id"]:
                        if isinstance(value, list):
                            raise Exception(
                                "Overwriting list of tagged data is not supported yet"
                            )
            ...
            for m_value in m_values:
                ...
                cur.execute(
                    cmd, [last_data_id, str(last_uid), member[0], m_value, prop_id]
                )
                # Add new band
                if band > 1:
            ...
            if name not in self.get_class().valid_properties_by_name[tag_clsname]:
                raise Exception(
                    '"%s" is not a valid property for class %s' % (name, tag_clsname)
                )
            ...
            if member is None:
                raise Exception(
                    "Unable to find membership for %s in %s" % (tag, self.meta["name"])
                )
            if self.coad.has_data_uid:
            ...
            if data_count == 0:
                ...
                if isinstance(value, list):
                    raise Exception(
                        "Attempting to set list for a single data property."
                    )
                data = all_data[0]
                cmd = "UPDATE data SET value=? WHERE data_id=?"
                ...
                if not isinstance(value, list):
                    raise Exception(
                        "Attempting to set a single value for a list data property."
                    )
                if len(value) != len(all_data):
                    raise Exception(
                        "Length of values passed in %s does not match set data list %s"
                        % (len(value), len(all_data))
                    )
                for val_idx, raw_val in enumerate(value):
                    # Data ids will already be in the correct order
                    cmd = "UPDATE data SET value=? WHERE data_id=?"
                    cur.execute(
                        cmd, [get_mask_value(raw_val, prop[0]), all_data[val_idx][0]]
                    )
                    self.coad.dbcon.commit()
        _logger.info(
            "set_property(%s, %s, %s) took %s sec",
            name,
            value,
            tag,
            time.time() - stime,
    ...
    def delete_property(self, name, tag="System.System"):
        ...
        for (data_id,) in all_data:
            ...
            cur.execute(
                "DELETE FROM text WHERE data_id=?",
                [
                    data_id,
                ],
            )
            cur.execute(
                "DELETE FROM tag WHERE data_id=?",
                [
                    data_id,
                ],
            )
            cur.execute(
                "DELETE FROM band WHERE data_id=?",
                [
                    data_id,
                ],
            )
    ...
    def tag_property(self, name, tag):
        ...
        _logger.info(
            "Data to be fed %s",
            [zip(all_data_ids, [tag_obj.meta["object_id"]] * len(all_data_ids))],
        )
        cur.executemany(
            cmd, zip(all_data_ids, [tag_obj.meta["object_id"]] * len(all_data_ids))
        )
        self.coad.dbcon.commit()
    ...
    def get_text(self):
        ...
        for parent_object_id, property_id, value, data_id in list(cur.fetchall()):
            ...
            name = self.get_class().valid_properties[parent.get_class().meta["name"]][
                str(property_id)
            ]["name"]
            # Check for tags
            tag_set = False
    ...
    def set_text(self, name, value, tag="System.System", class_id="Data File"):
        ...
        _logger.info(
            "Looking for collections for object %s name %s ",
            self.meta["object_id"],
            name,
        )
        ...
        for parent_object_id, membership_id, property_id in list(cur.fetchall()):
            ...
            if tag != "System.System" and tag != parent_obj.hierarchy:
                ...
                cur.execute(
                    cmd, [self.meta["object_id"], name, tag_obj.meta["object_id"]]
                )
            else:
            ...
            _logger.debug(
                "mem:%s prop:%s rc:%s", membership_id, property_id, len(match_data)
            )
            if len(match_data) < 1:
                default_value = self.get_class().valid_properties[
                    parent_obj.meta["name"]
                ][str(property_id)]["default_value"]
                ...
                cur.execute(
                    cmd, [str(last_uid + 1), membership_id, default_value, property_id]
                )
                _logger.debug(
                    cmd
                    + ":".join(
            ...
            if tag != "System.System" and tag != parent_obj.hierarchy:
                ...
                if len(cur.fetchall()) < 1:
                    ...
                    cur.execute(cmd, [data_id, tag_obj.meta["object_id"]])
                    _logger.debug(
                        cmd + ":".join([str(data_id), str(tag_obj.meta["object_id"])])
                    )
            self.coad.dbcon.commit()
        return
    ...
    def dump(self, recursion_level=0):
        ...
        msg = "Object:    {:<30}            ID: {}".format(
            self.meta["name"], self.meta["object_id"]
        )
        print(spacing + msg)
        msg = "    Class: {:<30}            ID: {}".format(
            self.get_class().meta["name"], self.meta["class_id"]
        )
        print(spacing + msg)
        msg = "    Category: {:<30}         ID: {}".format(
            self.get_category(), self.meta["category_id"]
        )
        print(spacing + msg)
        if self.keys():
        ...
        core = pandas.read_sql_query(
            core_cmd, self.coad.dbcon, params=[self.meta["object_id"]]
        )
        data_ids = core["data_id"].values.tolist()
        data_id_fmt = ",".join("?" * len(data_ids))
        try:
            band_cmd = (
                "SELECT data_id, band_id FROM band WHERE data_id IN (%s)" % data_id_fmt
            )
            band = pandas.read_sql_query(band_cmd, self.coad.dbcon, params=data_ids)
            core = core.merge(band, how="outer", on=["data_id"]).fillna(
                value={"band_id": 1}
            )
        except:
            ...
            core["band_id"] = 1
        try:
            date_from = pandas.read_sql_query(
                "SELECT data_id, date AS date_from FROM date_from WHERE data_id IN (%s)"
                % data_id_fmt,
                self.coad.dbcon,
                params=data_ids,
            )
            ...
            core["date_from"] = None
        try:
            date_to = pandas.read_sql_query(
                "SELECT data_id, date AS date_to FROM date_to WHERE data_id IN (%s)"
                % data_id_fmt,
                self.coad.dbcon,
                params=data_ids,
            )
        ...
        try:
            timeslice_cmd = (
                """SELECT data_id, value AS pattern FROM text t
                INNER JOIN class c ON c.class_id=t.class_id
                WHERE c.name='Timeslice' AND data_id IN (%s)"""
                % data_id_fmt
            )
            timeslice = pandas.read_sql_query(
                timeslice_cmd, self.coad.dbcon, params=data_ids
            )
        ...
        try:
            timeslice_obj_cmd = (
                """SELECT data_id, '{Object}'||o.name AS pattern FROM tag t
                INNER JOIN object o ON t.object_id=o.object_id
                INNER JOIN class c ON c.class_id=o.class_id AND c.name='Timeslice'
                WHERE data_id IN (%s)"""
                % data_id_fmt
            )
            timeslice_obj = pandas.read_sql_query(
                timeslice_obj_cmd, self.coad.dbcon, params=data_ids
            )
        ...
        try:
            datafile_cmd = (
                """SELECT data_id, value AS filename FROM text t
                INNER JOIN class c ON c.class_id=t.class_id
                WHERE c.name='Data File' AND data_id IN (%s)"""
                % data_id_fmt
            )
            datafile = pandas.read_sql_query(
                datafile_cmd, self.coad.dbcon, params=data_ids
            )
            core = core.merge(datafile, how="outer", on=["data_id"])
            datatag_cmd = (
                """SELECT t.data_id, 'Data File.'||o.name AS filename, t.object_id AS data_file_id FROM tag t
                INNER JOIN object o ON t.object_id=o.object_id
                INNER JOIN class c ON c.class_id=o.class_id AND c.name='Data File'
                WHERE data_id IN (%s)"""
                % data_id_fmt
            )
            datatag = pandas.read_sql_query(
                datatag_cmd, self.coad.dbcon, params=data_ids
            )
            ...
            datatagfile_cmd = (
                """SELECT t.data_id, t.value AS ref_filename, m.parent_object_id, m.child_object_id, m.collection_id  FROM data d
            INNER JOIN membership m ON m.membership_id=d.membership_id
            INNER JOIN property p ON p.property_id=d.property_id
            INNER JOIN text t ON t.data_id=d.data_id
            ...
            datatag_filenames = pandas.read_sql_query(
                datatagfile_cmd, self.coad.dbcon, params=datatag_ids
            )
        ...
        try:
            var_cmd = (
                """SELECT t.data_id, '{Object}'||o.name AS variable, a.action_symbol AS action FROM tag t
                INNER JOIN object o ON t.object_id=o.object_id
            ...
            try:
                var_cmd = (
                    """SELECT t.data_id, '{Object}'||o.name AS variable FROM tag t
                    INNER JOIN object o ON t.object_id=o.object_id
                    INNER JOIN class c ON c.class_id=o.class_id AND c.name='Variable'
                    WHERE data_id IN (%s)"""
                    % data_id_fmt
        ...
        try:
            scenario_cmd = (
                """SELECT t.data_id, 'Scenario.'||o.name AS scenario FROM tag t
                INNER JOIN object o ON t.object_id=o.object_id
                INNER JOIN class c ON c.class_id=o.class_id AND c.name='Scenario'
                WHERE data_id IN (%s)"""
                % data_id_fmt
            )
            scenario = pandas.read_sql_query(
                scenario_cmd, self.coad.dbcon, params=data_ids
            )
        ...
        try:
            memo_cmd = (
                """SELECT data_id, value as memo FROM memo_data WHERE data_id IN (%s)"""
                % data_id_fmt
            )
        ...
        sorted_props = core.sort_values(
            ["parent", "scenario", "property", "band_id"],
        ...
        for idx, row in sorted_props.iterrows():
            ...
            if not pandas.isna(row["filename"]):
                ...
                df_files = datatag_filenames.loc[
                    datatag_filenames["child_object_id"] == row["data_file_id"]
                ]
    ...
    def diff(self, other_obj):
        ...
        for key in self_props_keys & other_props_keys:
            if self_props[key] != other_props[key]:
                ...
                diff_msg.append(
                    "    Orig: %s Comp: %s" % (self_props[key], other_props[key])
                )
        # Children diff
